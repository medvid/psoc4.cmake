...
macro(psoc4_add_tools)
  ...
  if(NOT DEFINED CY_TOOLS_PATHS)
    ...
    if(NOT DEFINED TOOLS_VERSION)
      message(FATAL_ERROR "psoc4_add_tools: missing required VERSION argument.")
    endif()
  ...
  if(NOT IS_DIRECTORY ${CY_TOOLS_PATHS})
    message(FATAL_ERROR "psoc4_add_tools: CY_TOOLS_PATHS=${CY_TOOLS_PATHS} doesn't exist.")
  endif()
...
macro(psoc4_configure_toolchain)
...
macro(psoc4_configure_bsp)
  set(TARGET "" CACHE STRING "Target BSP")
  if("${TARGET}" STREQUAL "")
    message(FATAL_ERROR "psoc4_configure_bsp: TARGET is not set.")
  endif()

  set(_bsp_cmake ${CMAKE_CURRENT_SOURCE_DIR}/bsp/${TARGET}.cmake)
  if(NOT EXISTS ${_bsp_cmake})
    message(FATAL_ERROR "psoc4_configure_bsp: invalid TARGET: ${TARGET}.")
  endif()
  include(${_bsp_cmake})
  unset(_bsp_cmake)
endmacro()
...
macro(psoc4_configure_os)
  ...
  if(NOT EXISTS ${_os_cmake})
    message(FATAL_ERROR "psoc4_configure_os: invalid OS: ${OS}.")
  endif()
  include(${_os_cmake})
  unset(_os_cmake)
endmacro()
...
macro(psoc4_add_component component)
  list(APPEND COMPONENTS ${component})
  add_definitions(-DCOMPONENT_${component})
endmacro()

# Set target device MPN
macro(psoc4_set_device device)
  if("${device}" STREQUAL "")
    message(FATAL_ERROR "psoc4_set_device: missing required 'device' argument.")
  endif()
...
macro(psoc4_set_core)
  # Select target CPU core
  set(CORE CM0P CACHE STRING "Target CPU core")
  set_property(CACHE CORE PROPERTY STRINGS CM0P CM0)

  psoc4_add_component(${CORE})
...
macro(psoc4_fetch id url rev dir)
...
macro(psoc4_load_bsp)
  ...
  if(NOT DEFINED BSP_VERSION AND NOT DEFINED BSP_TAG)
    message(FATAL_ERROR "psoc4_load_bsp: missing VERSION or TAG argument.")
  endif()
...
macro(psoc4_load_library)
  ...
  psoc4_lib_name_to_prefix(${LIB_NAME} LIB_PREFIX)
  ...
  psoc4_fetch(${LIB_NAME} ${LIB_URL} ${LIB_TAG} ${LIB_DIR})
...
macro(psoc4_load_application)
  # Parse the expected one-value arguments
  cmake_parse_arguments(APP "" "NAME;VERSION;URL;TAG;DIR" "" ${ARGN})

  # NAME is the required argument
  if(NOT DEFINED APP_NAME)
    message(FATAL_ERROR "psoc4_load_application: missing required NAME argument.")
  endif()

  # Either VERSION or TAG should be provided
  if(NOT DEFINED APP_VERSION AND NOT DEFINED APP_TAG)
    message(FATAL_ERROR "psoc4_load_application: missing VERSION or TAG argument.")
  endif()
  ...
  psoc4_fetch(${APP_NAME} ${APP_URL} ${APP_TAG} ${APP_DIR})
...
macro(psoc4_add_design_modus design_modus var_source_dir var_sources)
  if(${ARGC} GREATER 3)
  ...
  if(NOT EXISTS ${design_modus})
    message(FATAL_ERROR "psoc4_add_design_modus: ${design_modus} doesn't exist.")
  endif()
...
macro(psoc4_add_design_capsense design_capsense var_source_dir var_sources)
  if(NOT EXISTS ${design_capsense})
    message(FATAL_ERROR "psoc4_add_design_capsense: ${design_capsense} doesn't exist.")
  endif()
...
macro(psoc4_add_design_seglcd design_seglcd var_source_dir var_sources)
  if(NOT EXISTS ${design_seglcd})
    message(FATAL_ERROR "psoc4_add_design_seglcd: ${design_seglcd} doesn't exist.")
  endif()
...
macro(psoc4_add_bsp_design_modus design_modus)
  psoc4_add_design_modus(
    ${design_modus}
...
macro(psoc4_add_bsp_design_capsense design_capsense)
  psoc4_add_design_capsense(
    ${design_capsense}
...
macro(psoc4_add_executable)
  # Parse the expected arguments
  cmake_parse_arguments(TARGET ""
    "NAME;DESIGN_MODUS;DESIGN_CAPSENSE;DESIGN_SEGLCD;LINKER_SCRIPT"
  ...
  if(NOT DEFINED TARGET_LINKER_SCRIPT)
    if(DEFINED BSP_LINKER_SCRIPT)
      ...
      message(FATAL_ERROR "psoc4_add_executable: specify either target LINKER_SCRIPT or BSP_LINKER_SCRIPT.")
    endif()
  endif()
